...
static void process_spmg(tcmotorRecord *prec)
{
    ...
    switch (prec->spmg) {
        ...
        case 1: /* Pause */
            halt = 1;
            prec->bmov = 0;  /* reset move-triggered flag */
            break;
...
static long init_record(struct dbCommon *pcommon, int pass)
{
     ...
     * Note: HOMF, HOMR, JOGF, JOGR are momentary pulse commands — the PLC
     * clears them immediately. Reading them back would overwrite the field
     * and cause repeated pulses, so they are intentionally excluded here. */
...
static long process(struct dbCommon *pcommon)
{
    ...
    if (!forwarded) {
        ...
        /* Jog increment readbacks — only update the last-sent cache,
         * not the JOGF/JOGR fields themselves (momentary pulses) */
        read_input(prec, &prec->rbk_jif, &prec->ljif);
        read_input(prec, &prec->rbk_jir, &prec->ljir);
    }
